...
BUILT_IN_FUNCTIONS = %w[
  DiscourseChatbot::StockDataFunction
...
module ::DiscourseChatbot
  class OpenAiBotRag < OpenAIBotBase
    ...
    def get_response(prompt, opts)
      ...
      if private_discussion
        ...
        system_message = {
          ...
          content:
            I18n.t(
              "chatbot.prompt.system.rag.open",
              current_date_time: DateTime.current
            )
        }
      end
    ...
    def merge_functions(opts)
      ...
      escalate_to_staff_function =
        ::DiscourseChatbot::EscalateToStaffFunction.new
      ...
      if SiteSetting.chatbot_locations_plugin_support &&
           defined?(Locations) == "constant" && Locations.class == Module &&
           defined?(::Locations::UserLocation) == "constant" &&
           ::Locations::UserLocation.class == Class &&
           ::Locations::UserLocation.count > 0
        user_search_from_location_function =
          ::DiscourseChatbot::ForumUserSearchFromLocationFunction.new
        user_search_from_user_location_function =
          ::DiscourseChatbot::ForumUserSearchFromUserLocationFunction.new
        get_coords_of_location_function =
          ::DiscourseChatbot::GetCoordsOfLocationDescriptionFunction.new
        user_distance_from_location_function =
          ::DiscourseChatbot::ForumUserDistanceFromLocationFunction.new
        get_distance_between_locations =
          ::DiscourseChatbot::GetDistanceBetweenLocationsFunction.new
          ...
          .each do |user_field|
            ...
            if !::UserCustomField.where(
                 ...
                 ::UserCustomField
                   ...
                   .value
                   .blank?
              functions << ::DiscourseChatbot::UserFieldFunction.new(
                user_field.name,
                opts[:user_id]
              )
            end
      ...
      if user_search_from_location_function
      ...
      if get_coords_of_location_function
        functions << get_coords_of_location_function
      end
      if user_distance_from_location_function
        functions << user_distance_from_location_function
      end
      if get_distance_between_locations
        functions << get_distance_between_locations
      end
      ...
      if !(
           SiteSetting.chatbot_serp_api_key.blank? &&
             SiteSetting.chatbot_jina_api_token.blank?
         )
        functions << web_search_function
      end
      if !SiteSetting.chatbot_marketstack_key.blank?
        functions << stock_data_function
      end

      if ::DiscourseChatbot::Function.descendants.count >
           BUILT_IN_FUNCTIONS.count
    ...
    def create_func_mapping(functions)
      ...
      functions.each_with_object({}) do |func, mapping|
        mapping[func.name] = func
      end
    end
    ...
    def create_chat_completion(messages, use_functions = true, iteration)
      begin
        ...
        additional_non_reasoning_parameters = {
          ...
          frequency_penalty:
            SiteSetting.chatbot_request_frequency_penalty / 100.0,
        ...
        if use_functions && @tools
          ...
          if iteration == 1
            if SiteSetting.chatbot_tool_choice_first_iteration ==
                 FORCE_A_FUNCTION
              parameters.merge!(tool_choice: "required")
            elsif SiteSetting.chatbot_tool_choice_first_iteration ==
                  FORCE_LOCAL_SEARCH_FUNCTION
              parameters.merge!(
                tool_choice: {
                  type: "function",
                  function: {
    ...
    def handle_function_call(res, opts)
      ...
      tools_called.each do |function_called|
        ...
        if func_name == "local_forum_search" || func_name == "escalate_to_staff"
          ...
          @posts_ids_found =
            (@posts_ids_found.to_set | (post_ids_found&.to_set || Set.new)).to_a
          @topic_ids_found =
            (
              @topic_ids_found.to_set | (topic_ids_found&.to_set || Set.new)
            ).to_a
          @non_post_urls_found =
            (
              @non_post_urls_found.to_set |
                (non_post_urls_found&.to_set || Set.new)
            ).to_a
        else
    ...
    def call_function(func_name, args_str, opts)
      ...
      begin
        ...
        if %w[escalate_to_staff remaining_bot_quota].include?(func_name)
          res, token_usage =
            func.process(args, opts).values_at(:answer, :token_usage)
        elsif ["vision"].include?(func_name)
          res, token_usage =
            func.process(args, opts, @client).values_at(:answer, :token_usage)
    ...
    def legal_post_urls?(res, post_ids_found, topic_ids_found)
      ...
      post_combos_in_text.each do |post_combo|
        ...
        post =
          ::Post.find_by(
            topic_id: topic_id_in_text.to_i,
            post_number: post_number_in_text.to_i
          )
    ...
    def legal_non_post_urls?(res, non_post_urls_found)
      ...
      urls_in_text.each do |url|
        return false if !non_post_urls_found.include?(url)
      end
      true
    end
    ...
    def image_url?(string)
      ...
      if image_extensions.any? { |ext| path.downcase.end_with?(ext) }
        return true
      end
      false
    end
