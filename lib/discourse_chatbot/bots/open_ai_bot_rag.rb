    ...
    class OpenAiBotRag < OpenAiBotBase
      ...
      def get_response(prompt, opts)
        ...
        if private_discussion
          system_message = {
            role: "developer",
            content:
              I18n.t("chatbot.prompt.system.rag.private", current_date_time: DateTime.current),
          }
        ...
        @inner_thoughts << {
          ...
          content:
            I18n.t(
              ...
              decision:
                I18n.t("chatbot.prompt.system.rag.advanced_local_reasoning.#{decision_key}"),
            ),
        }
      end
      ...
      def collect_trusted_url_provenance(tool_result)
        ...
        @trusted_url_provenance_collected ||=
          tool_result[:provenance_declared] || absolute_urls.present? ||
            relative_forum_paths.present?
      ...
      def response_urls_valid?(content)
        if !SiteSetting.chatbot_url_integrity_check || !@trusted_url_provenance_collected
          return true
        end
