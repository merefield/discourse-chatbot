...
module DiscourseChatbot
  module Post
    class PostEmbeddingProcess < ::DiscourseChatbot::EmbeddingProcess
      def upsert(post_id)
        if in_scope(post_id)
          if !is_valid(post_id)
      ...
      def semantic_search(query)
        ...
        begin
          ...
          Rails.logger.error("Error #{e} querying embeddings for search #{query}")
        ...
        if SiteSetting.chatbot_forum_search_tool_hybrid_search
          ...
          keyword_search_array_of_hashes =
            keyword_search.map do |id, user_id, score|
              { post_id: id, user_id: user_id, score: score, rank_modifier: 0, source: "keyword" }
            end
          ...
          keyword_search_array_of_hashes =
            keyword_search_array_of_hashes.each do |k|
              k[:score] = k[:score] / keyword_search_max_score * max_semantic_score
            end
