...
module DiscourseChatbot
  module Post
    class PostPromptUtils < ::DiscourseChatbot::PromptUtils
      def self.create_prompt(opts)
              ...
              [
                {
                  ...
                  content: I18n.t("chatbot.prompt.title", topic_title: current_topic.title),
                },
              ]
            else
              [
        ...
        if original_post_number == 1 &&
             (
               Array(
                 SiteSetting.chatbot_auto_respond_categories.split("|"),
               ).include? category_id.to_s
             ) &&
      ...
      def self.create_message(p, opts)
        post_content = p.raw
        bot_user_id = opts[:bot_user_id]
        post_content.gsub!(%r{\[quote.*?\](.*?)\[/quote\]}m, "") if SiteSetting.chatbot_strip_quotes
