...
module DiscourseChatbot
  module Message
    class MessageEvaluation < ::DiscourseChatbot::EventEvaluation
      ...
      def on_submission(submission)
        ...
        message_channel_user_count =
          ::Chat::UserChatChannelMembership.where(chat_channel_id: channel_id).count

        bot_chat_channel =
          (bot_user.user_chat_channel_memberships.where(chat_channel_id: channel_id).count > 0)

        talking_to_bot =
          (bot_chat_channel && message_channel_user_count < 3) ||
            (replied_to_user && replied_to_user.id == bot_user_id)
        ...
        if bot_user && (user_id != bot_user_id) && (mentions_bot_name || talking_to_bot)
          ...
          opts = {
            ...
            human_participants_count:
              bot_chat_channel ? message_channel_user_count - 1 : message_channel_user_count,
