        ...
        begin
          ...
          reset_job =
            MiniScheduler::Manager
              .discover_schedules
              .find do |job|
                job.schedule_info.instance_variable_get(:@klass).to_s == QUOTA_RESET_JOB
              end
              .schedule_info
          days_remaining =
            (
              Time.at(reset_job.instance_variable_get(:@next_run)).to_date - Time.zone.now.to_date
            ).to_i

          {
            answer:
              I18n.t(
                "chatbot.prompt.function.remaining_bot_quota.answer",
                quota: remaining_quota,
                units: SiteSetting.chatbot_quota_basis,
