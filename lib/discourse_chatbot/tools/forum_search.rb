    ...
    class ForumSearch < ::DiscourseChatbot::Tool
        ...
        [
          {
            name: "query",
            type: String,
            description: I18n.t("chatbot.prompt.function.forum_search.parameters.query"),
          },
          {
            name: "number_of_posts",
            type: Integer,
            description: I18n.t("chatbot.prompt.function.forum_search.parameters.number_of_posts"),
          },
        ]
      end
      ...
      def process(args)
        begin
          ...
          process_post_embedding = ::DiscourseChatbot::Post::PostEmbeddingProcess.new
          ...
          if SiteSetting.chatbot_forum_search_tool_results_content_type == "topic" ||
            ...
            top_topics_from_topic_title_results =
              top_topic_title_results.map { |result| result[:topic_id].to_i }.uniq
            top_topics = (top_topics_from_post_results + top_topics_from_topic_title_results).uniq

            response =
              I18n.t(
                "chatbot.prompt.function.forum_search.answer.topic.summary",
                number_of_topics: top_topics.length,
              )
            ...
            top_topics.each_with_index do |topic_id, index|
              ...
              top_topic_title_result =
                top_topic_title_results.find { |result| topic_id == result[:topic_id] }

              original_post_number = nil

              if top_post_result.present?
              ...
              response +=
                I18n.t(
                  "chatbot.prompt.function.forum_search.answer.topic.each.topic",
                  url: url,
                  title: title,
                  score: score,
                  rank: index + 1,
              ...
              while post_number <= max_post_number
                ...
                response +=
                  I18n.t(
                    "chatbot.prompt.function.forum_search.answer.topic.each.post",
                ...
                topic_ids_in_raw_urls_found, post_ids_in_raw_urls_found =
                  find_post_and_topic_ids_from_raw_urls(post.raw)
            ...
            response =
              I18n.t(
                "chatbot.prompt.function.forum_search.answer.post.summary",
            ...
            top_results.each_with_index do |result, index|
              ...
              score = result[:score]
              url =
                "https://#{Discourse.current_hostname}/t/slug/#{current_post.topic_id}/#{current_post.post_number}"
              raw = current_post.raw
              username = User.find(current_post.user_id).username
              date = current_post.created_at.to_date
              response +=
                I18n.t(
                  "chatbot.prompt.function.forum_search.answer.post.each",
                  url: url,
                  username: username,
                  date: date,
                  raw: raw,
              ...
              topic_ids_in_raw_urls_found, post_ids_in_raw_urls_found =
                find_post_and_topic_ids_from_raw_urls(raw)
          ...
          Rails.logger.error(
            "Chatbot: Error occurred while attempting to retrieve Forum Search results for query '#{query}': #{e.message}",
          )
          {
            answer: {
              result:
                I18n.t(
                  "chatbot.prompt.function.forum_search.error",
                  query: args[parameters[0][:name]],
                ),
              topic_ids_found: [],
              post_ids_found: [],
