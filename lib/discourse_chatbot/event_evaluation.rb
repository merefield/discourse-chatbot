  ...
  class EventEvaluation
    ...
    def over_quota(user_id)
      ...
      remaining_quota_field_name =
        (
          if SiteSetting.chatbot_quota_basis == "queries"
    ...
    def get_max_quota(user_id)
      ...
      GroupUser
        ...
        .each do |gu|
          if SiteSetting.chatbot_low_trust_groups.split("|").include? gu.group_id.to_s
            max_quota = SiteSetting.chatbot_quota_low_trust if max_quota <
              SiteSetting.chatbot_quota_low_trust
          end
          if SiteSetting.chatbot_medium_trust_groups.split("|").include? gu.group_id.to_s
            max_quota = SiteSetting.chatbot_quota_medium_trust if max_quota <
              SiteSetting.chatbot_quota_medium_trust
          end
          if SiteSetting.chatbot_high_trust_groups.split("|").include? gu.group_id.to_s
            max_quota = SiteSetting.chatbot_quota_high_trust if max_quota <
              SiteSetting.chatbot_quota_high_trust
          end
        end

      # deal with 'everyone' group
      max_quota = SiteSetting.chatbot_quota_low_trust if SiteSetting
        .chatbot_low_trust_groups
        .split("|")
        .include?("0") && max_quota < SiteSetting.chatbot_quota_low_trust
      max_quota = SiteSetting.chatbot_quota_medium_trust if SiteSetting
        .chatbot_medium_trust_groups
        .split("|")
        .include?("0") && max_quota < SiteSetting.chatbot_quota_medium_trust
      max_quota = SiteSetting.chatbot_quota_high_trust if SiteSetting
        .chatbot_high_trust_groups
        .split("|")
        .include?("0") && max_quota < SiteSetting.chatbot_quota_high_trust
    ...
    def escalate_as_required(user_id)
      ...
      if SiteSetting.chatbot_quota_reach_escalation_cool_down_period > 0
        if escalation_date.nil? ||
             !SiteSetting.chatbot_quota_reach_escalation_cool_down_period.nil? &&
               escalation_date.value <
                 SiteSetting.chatbot_quota_reach_escalation_cool_down_period.days.ago
