...
module DiscourseChatbot
  module Tasks
    def self.refresh_embeddings_all_sites(args)
      RailsMultisite::ConnectionManagement.each_connection { refresh_embeddings(args) }
    end

    def self.refresh_embeddings(args)
      puts "-" * 50
          ...
          Post
            ...
            .each do |post|
              if !missing_only.to_i.zero? &&
                   ::DiscourseChatbot::PostEmbedding.find_by(post_id: post.id).nil? ||
      ...
      begin
        ...
        process_topic_title_embedding = ::DiscourseChatbot::Topic::TopicTitleEmbeddingProcess.new
        ...
        (0..(total - 1).abs).step(batch) do |i|
          Topic
            ...
            .each do |topic|
              if !missing_only.to_i.zero? &&
                   ::DiscourseChatbot::TopicTitleEmbedding.find_by(topic_id: topic.id).nil? ||
                   missing_only.to_i.zero?
                process_topic_title_embedding.upsert(topic.id)
                sleep(delay) if delay
              end
              print_status(refreshed += 1, total)
            end
...
task "chatbot:refresh_embeddings", %i[missing_only delay] => :environment do |_, args|
  if ENV["RAILS_DB"]
    DiscourseChatbot::Tasks.refresh_embeddings(args)
  else
    DiscourseChatbot::Tasks.refresh_embeddings_all_sites(args)
  end
end
