...
System One request and response bodies are logged when verbose console logging is enabled or verbose Rails logging is set to `api_calls_only` or `all`. Entries include a correlation ID, response HTTP status and elapsed time; failures include the exception class. Authorization headers are omitted and the configured API key is redacted. These diagnostic bodies include question text and conversation context, including PM content, so enable verbose logging only when needed.
