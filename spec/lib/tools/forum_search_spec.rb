...
describe ::DiscourseChatbot::Tools::ForumSearch do
  ...
  let(:post_5) do
    Fabricate(
      :post,
      topic: topic_2,
      raw: "rains everywhere https://example.com/t/slug/#{post_2.topic_id}/#{post_2.post_number} ",
      post_number: 1,
    )
  end
  ...
  before(:each) do
    ::DiscourseChatbot::Post::PostEmbeddingProcess
      .any_instance
      .stubs(:semantic_search)
      .returns([{ post_id: post_3.id, score: 0.9 }, { post_id: post_5.id, score: 0.8 }])
    ::DiscourseChatbot::Post::PostEmbeddingProcess.any_instance.stubs(:in_scope).returns(true)
    ::DiscourseChatbot::Post::PostEmbeddingProcess.any_instance.stubs(:is_valid).returns(true)
  end
  ...
  it "returns contents of a high ranking Post" do
    ...
    expect(forum_search.process(args)[:answer][:topic_ids_found]).to eq(
      [post_3.topic_id, post_5.topic_id],
    )
    expect(forum_search.process(args)[:answer][:post_ids_found]).to include(post_5.id)
    expect(forum_search.process(args)[:answer][:post_ids_found]).to include(post_3.id)
    expect(forum_search.process(args)[:answer][:post_ids_found]).to include(post_2.id)
    expect(forum_search.process(args)[:answer][:post_ids_found]).not_to include(post_4.id)
    expect(forum_search.process(args)[:answer][:result]).to include(post_3.raw)
  end
  ...
  it "returns contents of a high ranking Topic" do
    ...
    expect(topic_2).not_to be_nil
    expect(topic_3).not_to be_nil
    expect(forum_search.process(args)[:answer][:topic_ids_found]).to include(topic_1.id)
    expect(forum_search.process(args)[:answer][:result]).to include(post_1.raw)
    expect(forum_search.process(args)[:answer][:result]).to include(post_2.raw)
    expect(forum_search.process(args)[:answer][:result]).to include(post_3.raw)
    expect(forum_search.process(args)[:answer][:result]).to include(topic_1.title)
    expect(forum_search.process(args)[:answer][:result]).not_to include(topic_3.title)
    expect(forum_search.process(args)[:answer][:result]).not_to include(post_4.raw)
  end

  it "finds urls with a post id" do
    expect(forum_search.find_post_and_topic_ids_from_raw_urls(post_5.raw)).to eq(
      [[post_2.topic_id], [post_2.id]],
    )
  end
  ...
  it "method finds urls that are not posts" do
    ...
    TEXT
    result = forum_search.find_other_urls(text)
    expect(result.length).to eq(3)
    expect(result).to eq(
      %w[
        https://example.com
        http://another-example.org/path?query=string
        https://www.sample.org/sample-path
      ],
    )
  end
