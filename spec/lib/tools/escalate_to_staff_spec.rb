...
describe ::DiscourseChatbot::Tools::EscalateToStaff do
  ...
  before { SiteSetting.chatbot_bot_user = chatbot_user.username }

  it "returns an error if fired from a Post" do
    opts = { type: ::DiscourseChatbot::POST }

    expect(escalate_to_staff.process({}, opts)).to eq(
  ...
  it "creates a Private Message and stores escalation topic id" do
    ...
    described_class.any_instance.stubs(:get_messages).returns(%w[this is a test])
    ...
    expect { escalate_to_staff.process({}, opts) }.to change { Topic.count }.by(1)
    expect(Topic.last.title).to eq(
      "#{I18n.t("chatbot.prompt.function.escalate_to_staff.title")}: Test enquiry",
    )
  ...
  it "returns existing escalation topic when within cooldown period" do
    ...
    result = nil
    expect { result = escalate_to_staff.process({}, opts) }.not_to change { Topic.count }
