...
describe ::DiscourseChatbot::Post::PostPromptUtils do
  ...
  it "excludes bot inner-thought posts without excluding staff whispers or consuming history capacity" do
    ...
    staff_whisper =
      Fabricate(:post, topic: topic, user: staff_user, post_type: Post.types[:whisper])
    inner_thoughts_raw = ::DiscourseChatbot::INNER_THOUGHTS_POST_PREFIX + "[]\n```\n[/details]"
    ...
    inner_thoughts_regular = Fabricate(:post, topic: topic, user: bot_user, raw: inner_thoughts_raw)
    ...
    expect(serialized_prompt).not_to include(inner_thoughts_whisper.raw, inner_thoughts_regular.raw)
